...
/**
 * Extract target role/job title from job description
 */
export function extractTargetRole(jobDescription: string): string | null {
  ...
  for (const { pattern, role } of rolePatterns) {
    if (pattern.test(jobDescription)) {
      console.log(`✓ Extracted target role: ${role}`);
      return role;
    }
  }
  ...
  if (firstLine && firstLine.length > 5 && firstLine.length < 100) {
    // Likely a job title
    console.log(`✓ Using first line as target role: ${firstLine}`);
    return firstLine;
  }
  
  // No role found
  console.log('⚠️  Could not extract target role from job description');
  return null;
}
